...
if [ ! -w "$bindir" ]; then
  echo "It seems you do not have a permission to install 'sq' executable file to $bindir." 2>&1
  echo "Please run this script again with appropriate permissions." 2>&1
  exit 1
fi

if \sq --version > /dev/null 2>&1; then
  if [ "$( \command -v sq )" != "$bindir/sq" ] || [ -L "$bindir/sq" ]; then
    echo 'sq is already installed by using another method.' 2>&1
    echo 'Please use the same method if you want to update sq.' 2>&1
    exit 1
  fi
fi

if ! \curl --version > /dev/null 2>&1; then
  echo '"curl" command is required to install sq' 2>&1
...
url="$( \curl -fsSL https://api.github.com/repos/bitbears-dev/sq/releases/latest | \
